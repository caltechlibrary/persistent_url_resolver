...
def make_link_history(collection,resolver,url,note):
    ...
    #Run link check
    get = requests.get('http://resolver.library.caltech.edu/'+resolver)#'https://ddlgr1hc96u88.cloudfront.net/'+resolver)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-update",action='store_true',
        help="Update ALL (not just new) resolver links",
    )
    ...
    #Get DOI links
    if args.dois:
        client_ids =['tind.caltech','caltech.library','caltech.ipacdoi','caltech.micropub','caltech.hte']
    ...
    #Get Eprints links
    repos = [('datawork@caltechconf.library.caltech.edu','./purr_caltechconf.sql'),
            ('datawork@caltechcampuspubs.library.caltech.edu','./purr_caltechcampuspubs.sql'),	
